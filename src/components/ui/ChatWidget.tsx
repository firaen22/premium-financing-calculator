...
type Message = { role: 'user' | 'assistant'; content: string; toolCalls?: { name: string }[] };
type ApiResponse = { reply: string; toolCalls?: { name: string }[]; error?: string };

export const ChatWidget = () => {
    const { t, lang, simulationInput } = useApp();
    ...
    const send = async (text: string, retry = false) => {
        ...
        try {
            ...
            setMessages(value => [...value, { role: 'assistant', content: data.reply, toolCalls: data.toolCalls }]);
    ...
    return <>
        ...
        {open && <section role="dialog" aria-modal="false" aria-label={t.chat.title} className="fixed bottom-5 right-5 z-50 flex h-[min(34rem,calc(100vh-2rem))] w-[min(24rem,calc(100vw-2rem))] flex-col overflow-hidden rounded-lg border border-[#c5a05966] bg-white shadow-2xl">
            ...
            <div className="flex-1 space-y-3 overflow-y-auto p-4" aria-live="polite">
                ...
                {messages.map((message, index) => <div key={`${index}-${message.role}`} className={`rounded-lg px-3 py-2 text-sm ${message.role === 'user' ? 'ml-8 bg-[#fffaf0] text-slate-700' : 'mr-4 bg-slate-50 text-slate-700'}`}><p className="whitespace-pre-wrap">{message.content}</p>{message.toolCalls?.length ? <p className="mt-2 text-xs text-slate-400">{t.chat.computedVia}: {message.toolCalls.map(call => call.name).join(', ')}</p> : null}</div>)}
